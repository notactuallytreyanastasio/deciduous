...
description: Sync decision graph with teammates - reconcile .deciduous/graph.json with the local database
...
This does, in order:
1. Creates `.deciduous/graph.json` if it does not exist yet
...
/// PreToolUse hook: counts unlogged work and denies the call past the limit.
/// A wrapper, so installs whose settings.json already points at this path
/// get the current rules from the binary on every `deciduous update`.
pub const HOOK_REQUIRE_ACTION_NODE: &str = r#"#!/bin/sh
# require-action-node.sh (PreToolUse: Edit|Write|NotebookEdit|Bash)
# Generated by deciduous. The rules live in `deciduous log-loop`: an agent
# that has done more than DECIDUOUS_LOG_EVERY (10) things since its last graph
# write, or committed without logging, is told to log before it continues.
command -v deciduous >/dev/null 2>&1 || exit 0
# Not exec: a binary older than 1.0.2 has no log-loop and exits 2, which
# Claude Code reads as "block this tool call". Only a failure of the
# binary is swallowed; the hook's own verdicts are JSON with exit 0.
deciduous log-loop pre || exit 0
"#;

/// PostToolUse hook on Bash: notices commits and CLI graph writes.
pub const HOOK_POST_COMMIT_REMINDER: &str = r#"#!/bin/sh
# post-commit-reminder.sh (PostToolUse: Bash)
# Generated by deciduous. A `deciduous add|link|...` counts as logging; a git
# commit or merge must be logged before the agent's next action.
command -v deciduous >/dev/null 2>&1 || exit 0
# Not exec: a binary older than 1.0.2 has no log-loop and exits 2, which
# Claude Code reads as "block this tool call". Only a failure of the
# binary is swallowed; the hook's own verdicts are JSON with exit 0.
deciduous log-loop post-bash || exit 0
"#;
