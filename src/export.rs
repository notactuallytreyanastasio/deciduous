...
//!
//! Provides DOT, Mermaid graph export and PR writeup generation.
...
fn mermaid_node_shape(node_type: &str, id: i32, label: &str) -> String {
    let escaped = escape_mermaid(label);
    match node_type {
        "goal" => format!("{}[/{}\\]", id, escaped),           // Trapezoid
        "decision" => format!("{}{{{{ {} }}}}", id, escaped),   // Diamond (rhombus)
        "option" => format!("{}[/ {} /]", id, escaped),         // Parallelogram
        "action" => format!("{}([{}])", id, escaped),           // Stadium (pill)
        "outcome" => format!("{}[[{}]]", id, escaped),          // Subroutine box
        "observation" => format!("{}>{}]", id, escaped),        // Flag shape
        _ => format!("{}[{}]", id, escaped),                    // Rectangle
    }
}

/// Get Mermaid style class for node type
fn mermaid_node_class(node_type: &str) -> &'static str {
    match node_type {
        "goal" => "goal",
        "decision" => "decision",
        "option" => "option",
...
/// Escape string for Mermaid labels
...
/// Convert a decision graph to Mermaid format
pub fn graph_to_mermaid(graph: &DecisionGraph, config: &MermaidConfig) -> String {
    let mut mermaid = String::new();

    // Graph header
    writeln!(mermaid, "flowchart {}", config.direction).unwrap();

    // Add title as a comment if provided
    if let Some(title) = &config.title {
        writeln!(mermaid, "    %% {}", title).unwrap();
    }
    ...
    for node in &graph.nodes {
        ...
        label.push_str(&truncate(&node.title, 35));

        if config.show_confidence {
            if let Some(conf) = extract_confidence(&node.metadata_json) {
                write!(label, " ({}%)", conf).unwrap();
            }
        }

        let node_def = mermaid_node_shape(&node.node_type, node.id, &label);
        writeln!(mermaid, "    {}", node_def).unwrap();
    }
    ...
    for edge in &graph.edges {
        ...
        if let Some(rationale) = &edge.rationale {
            let short = truncate(rationale, 20);
            writeln!(mermaid, "    {} {}|{}| {}", edge.from_node_id, arrow, escape_mermaid(&short), edge.to_node_id).unwrap();
        } else {
            writeln!(mermaid, "    {} {} {}", edge.from_node_id, arrow, edge.to_node_id).unwrap();
        }
    }
    ...
    // Style classes
    writeln!(mermaid, "    classDef goal fill:#FFE4B5,stroke:#DAA520,stroke-width:2px").unwrap();
    writeln!(mermaid, "    classDef decision fill:#E6E6FA,stroke:#9370DB,stroke-width:2px").unwrap();
    writeln!(mermaid, "    classDef option fill:#E0FFFF,stroke:#20B2AA,stroke-width:1px").unwrap();
    writeln!(mermaid, "    classDef action fill:#90EE90,stroke:#228B22,stroke-width:2px").unwrap();
    writeln!(mermaid, "    classDef outcome fill:#87CEEB,stroke:#4169E1,stroke-width:2px").unwrap();
    writeln!(mermaid, "    classDef observation fill:#DDA0DD,stroke:#8B008B,stroke-width:1px,stroke-dasharray: 5 5").unwrap();

    // Apply classes to nodes
    for node in &graph.nodes {
        writeln!(mermaid, "    class {} {}", node.id, mermaid_node_class(&node.node_type)).unwrap();
    }
