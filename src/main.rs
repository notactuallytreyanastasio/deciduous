...
enum Command {
    ...
    Init {
        /// Bootstrap for VS Code agent environment
        #[arg(long)]
        code: bool,

        /// Bootstrap for Claude environment (default)
        #[arg(long)]
        claude: bool,
    },
...
fn main() {
    ...
    if let Command::Init { code, claude } = args.command {
        let env = if code {
            Some(deciduous::init::InitEnv::Code)
        } else if claude {
            Some(deciduous::init::InitEnv::Claude)
        } else {
            None
        };

        if let Err(e) = deciduous::init::init_project(env) {
    ...
    match args.command {
        ...
        Command::Nodes => match db.get_all_nodes() {
            Ok(nodes) => {
                if nodes.is_empty() {
