...
This reference describes Deciduous 1.0. Start with [local Postgres setup](local-postgres.md), then [connect your agents](clients.md). The [MCP guide](mcp.md) explains the transport and tool differences.
...
For released `v1.0.0` installations, use the current Python helper from the [Postgres-only setup guide](local-postgres.md#generate-just-postgres). Both generators use the same templates. Follow the generated README before starting containers or connecting an app.
...
The header is a routing safeguard, not an access-control boundary. The shared bearer token grants access to the service's data. `list_workspaces` and ID-based reads are not confined by a workspace header. See [security boundaries](architecture.md#security-boundaries).
...
`remote push` can overwrite newer server fields with a stale local copy. `remote pull` does not reconcile server deletions into a faithful local mirror. Read [upgrading and migrating](upgrading.md) before using either on valuable history.
...
See [architecture](architecture.md) for persistence and security limits, or [solo and offline use](solo.md) for the local CLI and Git-based workflow.
