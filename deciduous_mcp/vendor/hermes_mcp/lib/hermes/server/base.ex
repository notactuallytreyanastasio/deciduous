defmodule Hermes.Server.Base do
  ...
  def init(%{module: module} = opts) do
    ...
    state = %{
      ...
      frame: Frame.new(),
      server_requests: %{},
      # task ref => %{from, session, request_id, method} for every request
      # whose handler is running outside this process (start_request_task/4).
      pending_requests: %{}
    }
  ...
  def handle_call({:request, decoded, session_id, context}, from, state) when is_map(decoded) do
    with {:ok, {%Session{} = session, state}} <-
      ...
      case handle_single_request(decoded, session, state) do
        # A handler-backed request (tools/call, tools/list, prompts/*, ...)
        # runs in a Task, not in this process. There is one Base per server
        # module and every session's requests pass through it, so while one
        # handler ran inline here every other session's call waited behind it.
        # The reply goes back through GenServer.reply/2 when the task ends.
        {:async, request, new_state} ->
          start_request_task(request, session, from, new_state)

        # initialize, ping, logging/setLevel and the protocol errors are cheap
        # and touch this process's state, so they still answer inline.
        {:reply, reply, new_state} ->
          complete_request(session, reply_id(reply, decoded["id"]))
          {:reply, encode_reply(reply, decoded["id"]), new_state}
      end
    end
  end
  ...
  # The id under which the session tracked this request. Real replies carry
  # the request's own id; the not-initialized error carries a generated one,
  # for which complete_request/2 is a no-op, as it was before this patch.
  defp reply_id({:ok, %{"id" => id}}, _request_id), do: id
  defp reply_id(_reply, request_id), do: request_id

  defp complete_request(_session, nil), do: :ok
  defp complete_request(session, request_id), do: Session.complete_request(session.name, request_id)

  # Pure: turns a handler reply into the bytes the transport sends. It runs in
  # the handler's task for async requests, because for a large result this is
  # not cheap: the JSON-RPC envelope re-escapes the whole result string, which
  # for a 33MB get_graph measured 400-640ms. Left in this process that was the
  # one serial section still shared by every session.
  defp encode_reply({:ok, %{"result" => result} = response}, _request_id) do
    Message.encode_response(%{"result" => result}, response["id"])
  end

  defp encode_reply({:ok, %{"error" => error} = response}, _request_id) do
    Message.encode_error(%{"error" => error}, response["id"])
  end

  # A handler answered {:noreply, frame}. Nothing on the HTTP path can deliver
  # a later reply, and before this patch the same value crashed this process
  # with a CaseClauseError. Answer the request with an error instead.
  defp encode_reply({:ok, nil}, request_id) do
    error = Error.execution("handler returned :noreply; this transport cannot deliver a deferred reply")
    Message.encode_error(Error.build_json_rpc(error, request_id), request_id)
  end

  defp encode_reply({:error, error}, _request_id), do: {:error, error}

  # Runs module.handle_request/2 under the Task.Supervisor that
  # Hermes.Server.Supervisor already starts for the transport, and parks the
  # caller until the task reports back (handle_info below).
  #
  # The task gets the frame populate_frame/4 just built for this session and
  # request. It is not written back into state afterwards: populate_frame/4
  # rebuilds the transport and private sections for every request anyway, and
  # with several handlers in flight a write-back would only race.
  defp start_request_task(%{"id" => request_id} = request, session, from, %{module: module, frame: frame} = state) do
    supervisor = state.registry.task_supervisor(module)

    task =
      Task.Supervisor.async_nolink(supervisor, fn ->
        {reply, _frame} = server_request(request, module, frame)
    ...
    {:noreply,
     %{
       state
       | pending_requests: Map.put(state.pending_requests, task.ref, info),
  ...
  # A handler task finished and already encoded its reply: bookkeeping, reply.
  @impl GenServer
  ...
  # A handler task crashed. Inline, the same exception took this process down
  # and, through the one_for_all supervisor, every session with it (seen in
  # production 2026-09-22: an add_edge with from_node_id "" logged out every
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, %{pending_requests: pending} = state)
    ...
    Logging.server_event(
      ...
      %{id: request_id, method: method, reason: inspect(reason, pretty: true)},
      level: :error
    )
  ...
  defp handle_request(%{"id" => request_id, "method" => method} = request, session, state) do
    ...
    {:async, request, %{state | frame: frame}}
  end
  ...
  # Runs inside a Task.Supervisor child (start_request_task/4), never in this
  # GenServer. Returns {reply, frame}; encode_reply/2 accepts the reply.
  defp server_request(%{"id" => request_id, "method" => method} = request, module, frame) do
    case module.handle_request(request, frame) do
      {:reply, response, %Frame{} = frame} ->
        ...
        {{:ok, Message.build_response(response, request_id)}, frame}
      ...
      {:error, %Error{} = error, %Frame{} = frame} ->
        ...
        {{:ok, Error.build_json_rpc(error, request_id)}, frame}
    end
  end
