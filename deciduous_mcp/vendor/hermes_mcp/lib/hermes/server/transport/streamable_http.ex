defmodule Hermes.Server.Transport.StreamableHTTP do
  ...
  # `:infinity` here is safe: the transport itself bounds the request with
  # `state.request_timeout` (30s default) in `forward_request_to_server/6`, and
  # replies to the caller from `handle_info` when the Task finishes or dies.
  # With Erlang's default 5_000 the Plug process exited (HTTP 500, empty body)
  # while Base kept running the abandoned tool call, so every session behind
  # it paid for work nobody would receive.
