...
test "filters active users" do
  ...
  result = Users.list_active()

  assert [^active] = result
end
```

## Ecto Fragment Escaping

When testing PostgreSQL JSONB queries with `?` operator:

```elixir
# The ? must be escaped as \\? in fragments
from(n in Node,
  where: fragment("? \\? ?", n.metadata, ^key)
)
```
...
# GOOD - use assert_receive or monitors
assert_receive {:event, :completed}, 5000

# Or synchronize with :sys.get_state
_ = :sys.get_state(pid)  # Forces sync
```

## DON'T: Access Lists by Index

```elixir
# BAD - lists don't support index access
mylist[0]  # This doesn't work!
...
## DON'T: Forget Immutability in Blocks

```elixir
# BAD - rebinding inside if has no effect outside
if condition do
...
**Now implement $ARGUMENTS.FEATURE using TDD:**

1. Create test file at correct mirror path
