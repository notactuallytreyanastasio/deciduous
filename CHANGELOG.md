...
### Changed
- **`DB_SSL=true` now verifies the database certificate and hostname.** In 1.0.1 it encrypted without checking who answered (`verify_none`). A deployment that sets `DB_SSL=true` against a private CA should add `DB_SSL_CA_FILE`; one whose `DATABASE_URL` names an IP address, or that cannot verify at all, sets `DB_SSL_VERIFY=none` to keep the 1.0.1 behaviour. With `DB_SSL` unset, TLS stays off, as the compose setup expects.
